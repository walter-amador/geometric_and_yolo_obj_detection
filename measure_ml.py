...
print("Press 'q' to quit (or will auto-stop after 1 minute)")
...
measurement_duration = 60  # 1 minute in seconds
...
while True:
    ...
    if elapsed_time >= measurement_duration:
        print(f"\n1 minute elapsed. Stopping measurement...")
        break
...
if fps_log:
    with open(csv_filename, 'w', newline='') as csvfile:
        ...
        writer.writeheader()
        writer.writerows(fps_log)
    
    # Calculate statistics
    fps_values = [entry['fps'] for entry in fps_log]
    ...
    print(f"  Duration: {len(fps_log)} frames over {fps_log[-1]['timestamp']:.2f} seconds")
    print(f"  Average FPS: {avg_fps:.2f}")
    print(f"  Min FPS: {min_fps:.2f}")
    print(f"  Max FPS: {max_fps:.2f}")
    print(f"  Std Dev: {std_fps:.2f}")
    print(f"\nFPS data saved to: {csv_filename}")
